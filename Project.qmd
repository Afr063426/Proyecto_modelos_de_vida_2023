...
# 
# 
# n<-predict(arbol1, type = "node" )
# 
# 
# 
# a<-survfit(Surv(time, DEATH_EVENT)~1, data = baseDatos80, subset = (n==prediccionProb[1]) )
# 
# prediccionProb[1]
# 
# summary(a)
...
a<-ggsurvplot(sf,ggtheme = theme_minimal(), font.family = "Times New Roman", censor.shape=124,censor.name  = "Censura", 
              palette = c("#FF6384", "#36A2EB",  "#FFCE56", "#4BC0C0"))

b<-a$plot+geom_text(aes(x = 350, y = .8, label = "Texto en la leyenda"), 
                   show.legend = FALSE)+
  labs(x="Tiempo en días", y ="Probabilidad de supervivencia", color = "Nodos", shape  = "Censura")+
  theme(legend.position = "right")


ggsave(filename="Nodos.pdf",plot = b, width = 210  ,
  height = 297  , units = "mm" )
...
c <- Cindex(Surv(baseDatos20$time, baseDatos20$DEATH_EVENT), prediccionProb)
